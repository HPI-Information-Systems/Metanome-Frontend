...
app.controller('ResultCtrl', function ($scope, $log, Executions, Results, $q, usSpinnerService,
  ...
  function init() {
    if ($scope.ucc || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.uniqueColumnCombination.params.type).
    ...
    if ($scope.fd || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.functionalDependency.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
    ...
    if ($scope.basicStat || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.basicStatistic.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
    ...
    if ($scope.ind || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.inclusionDependency.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
    ...
    if ($scope.cucc || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.conditionalUniqueColumnCombination.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
    ...
    if ($scope.od || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.orderDependency.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
    ...
    if ($scope.mvd || $scope.file) {
      $http.get(ENV_VARS.apiURL + '/api/result-store/count/' + $scope.multivaluedDependency.params.type).
        then(function (response) {
          var count = response.data;
          if (count > 0) {
